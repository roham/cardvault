...
const _teamRosters: Record<string, string[]> = {
  ...
  'houston-astros': ['Alex Bregman', 'Jose Altuve', 'Yordan Alvarez', 'Framber Valdez', 'Jeff Bagwell', 'Craig Biggio', 'Nolan Ryan', 'Roger Clemens', 'Hunter Brown', 'Spencer Arrighetti', 'Yainer Diaz'],
  ...
  'new-orleans-pelicans': ['Zion Williamson', 'Brandon Ingram', 'Trey Murphy III', 'Herb Jones', 'Yves Missi'],
  ...
  'pittsburgh-steelers': ['Terry Bradshaw', 'Mean Joe Greene', 'Troy Polamalu', 'Ben Roethlisberger', 'T.J. Watt', 'Franco Harris', 'Jack Lambert', 'Jack Ham', 'John Stallworth', 'Mel Blount', 'George Pickens', 'Najee Harris', 'Darnell Washington', 'Troy Fautanu'],
  ...
  'minnesota-vikings': ['Adrian Peterson', 'Fran Tarkenton', 'Alan Page', 'Justin Jefferson', 'John Randle', 'Cris Carter', 'Randall Cunningham', 'Anthony Carter', 'Sam Darnold', 'J.J. McCarthy', 'Dallas Turner'],
  ...
  'washington-capitals': ['Alex Ovechkin', 'Alexander Ovechkin', 'Nicklas Backstrom', 'Rod Langway', 'Mike Gartner', 'Tom Wilson', 'Connor McMichael'],
  ...
  'columbus-blue-jackets': ['Rick Nash', 'Johnny Gaudreau', 'Kirill Marchenko', 'Yegor Chinakhov', 'Adam Fantilli', 'Kent Johnson', 'Dmitri Voronkov', 'Cayden Lindstrom'],
...
// Wave 5 — v4.3 additions
const _wave5: Record<string, string[]> = {
...
// Wave 6 — v4.5 additions
const _wave6: Record<string, string[]> = {
  'baltimore-orioles': ['Chris Davis', 'Adam Jones'],
...
for (const [teamSlug, players] of Object.entries(_wave5)) {
  if (!_teamRosters[teamSlug]) _teamRosters[teamSlug] = [];
  _teamRosters[teamSlug].push(...players);
}
...
// Wave 11: v4.15 additions
const _wave11: Record<string, string[]> = {
...
// Wave 15 — v4.22
const _wave15: Record<string, string[]> = {
  'oakland-athletics': ['Brent Rooker'],
  'cleveland-cavaliers': ['Jaylon Tyson', 'Isaac Okoro'],
  'memphis-grizzlies': ['Jaylen Wells'],
  'miami-heat': ["Kel'el Ware"],
  'new-york-jets': ['Braelon Allen'],
  'pittsburgh-steelers': ['Mason McCormick'],
  'st-louis-blues': ['Jake Neighbors'],
  'washington-capitals': ['Ivan Miroshnichenko'],
};
for (const [teamSlug, players] of Object.entries(_wave15)) {
  if (!_teamRosters[teamSlug]) _teamRosters[teamSlug] = [];
  _teamRosters[teamSlug].push(...players);
}

// Wave 16 — v4.23
const _wave16: Record<string, string[]> = {
  ...
  'new-york-rangers': ['Filip Chytil', 'Will Cuylle'],
};
for (const [teamSlug, players] of Object.entries(_wave16)) {
  if (!_teamRosters[teamSlug]) _teamRosters[teamSlug] = [];
  _teamRosters[teamSlug].push(...players);
}

// Wave 17 — v4.24
const _wave17: Record<string, string[]> = {
  'washington-nationals': ['Jake Irvin'],
  'seattle-mariners': ['Andres Munoz'],
  'texas-rangers': ['Kirby Yates'],
  'atlanta-falcons': ['Kyle Pitts', 'Chauncey Gardner-Johnson'],
  'san-francisco-49ers': ['Talanoa Hufanga'],
  'brooklyn-nets': ['Nicolas Claxton'],
  'new-york-knicks': ['Precious Achiuwa'],
  'florida-panthers': ['Aaron Ekblad', 'Evan Rodrigues'],
};
for (const [teamSlug, players] of Object.entries(_wave17)) {
  if (!_teamRosters[teamSlug]) _teamRosters[teamSlug] = [];
  _teamRosters[teamSlug].push(...players);
}
...
// Wave 21: v4.27 — 10 new players
const _wave21: Record<string, string[]> = {
  'chicago-cubs': ['Javier Assad', 'Ben Brown'],
  'kansas-city-chiefs': ['Kadarius Toney', 'Skyy Moore'],
...
for (const [teamSlug, players] of Object.entries(_wave21)) {
  if (!_teamRosters[teamSlug]) _teamRosters[teamSlug] = [];
  _teamRosters[teamSlug].push(...players);
}

// Wave 22: v4.28 — 10 new players
const _wave22: Record<string, string[]> = {
  'florida-marlins': ['Josh Beckett'],
  'houston-astros': ['Roy Oswalt'],
  'philadelphia-76ers': ['Jerry Stackhouse'],
  'seattle-supersonics': ['Rashard Lewis'],
...
for (const [teamSlug, players] of Object.entries(_wave22)) {
  if (!_teamRosters[teamSlug]) _teamRosters[teamSlug] = [];
  _teamRosters[teamSlug].push(...players);
}

// Wave 41: v4.45 — 10 new players
const _wave41: Record<string, string[]> = {
  'new-york-mets': ['Tylor Megill'],
  'oakland-athletics': ['Joe Boyle'],
  'milwaukee-brewers': ['Robert Gasser'],
  'dallas-cowboys': ['Ollie Gordon II'],
  'new-england-patriots': ["Ja'Lynn Polk"],
  'indiana-pacers': ['Sandro Mamukelashvili'],
...
for (const [teamSlug, players] of Object.entries(_wave41)) {
  if (!_teamRosters[teamSlug]) _teamRosters[teamSlug] = [];
  _teamRosters[teamSlug].push(...players);
}

// Wave 43: v4.46 — 10 new players
const _wave43: Record<string, string[]> = {
  'new-york-mets': ['Blade Tidwell'],
  'cleveland-guardians': ['Bo Naylor'],
  'cincinnati-reds': ['Nick Martinez'],
  'kansas-city-chiefs': ['Isaiah Pacheco'],
  'carolina-panthers': ['Terrace Marshall Jr.'],
  'sacramento-kings': ['Colby Jones'],
  'atlanta-hawks': ['Nacho Alvarez Jr.'],
